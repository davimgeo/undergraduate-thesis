...
int main()
{
  ...
  for (int i = 0; i < SIZE; i++)
  {
    ...
    geometry_t* grad_geom = Geometry_InitCreate(
      NULL,
      &specs->geometry
    );
    ...
    Geometry_Create(
      grad_geom,
      GEOMETRY_ONLY_RECEIVERS
    );

    int irec = i * (grad_geom->nrec - 1) / (SIZE - 1);

    int src_x = grad_geom->rec.x[irec];
    ...
    if (i == 1)
      plot_model_geometry(grad_model_obj, 10, grad_geom);
    ...
    printf(
      "Alpha: %g | Rec index: %d | Src x: %d | "
      "L2: %g | L1: %g | Cross: %g | Decon: %g\n",
      alphas[i],
      irec,
      src_x,
      l2[i],
