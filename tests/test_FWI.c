...
float* get_nabla_gradient(float* mk, const float* dobs, SpecsContext* specs)
{
  ...
  size_t model_size = (size_t)model->nz * model->nx;
...
double l2_norm(const float* dcalc, const float* dobs, int nt, int nrec, int nshot)
{
  double result = 0.0;
...
double gradient_norm(const float* gradient, size_t size)
{
  double result = 0.0;
...
double get_GTP(const float* nabla_chi, double grad_norm, size_t size)
{
  double gTp = 0.0;

  for (size_t i = 0; i < size; i++) 
  {
    double h_k = -(double)nabla_chi[i] / grad_norm;
...
int main()
{
  ...
  //float* m8 = read_any("data/FWI/m_008.bin", 881 * 351);
  //plot2d(m8, 351, 881);
  ...
  for (int it = 0; it < MAX_ITERATIONS; it++) 
  {
    ...
    double grad_norm = gradient_norm(nabla_chi, model_size);
    double gTp = get_GTP(nabla_chi, grad_norm, model_size);
    ...
    for (int ils = 0; ils < MAX_LINE_SEARCH; ++ils)
    {
      ...
      {
        // m_{k+1} = m_k - a_k \nabla\chi(m_k)
        mk1[i] = mk[i] - a_k * (float)((double)nabla_chi[i] / grad_norm);
      }
      ...
      if (armijo)
      {
        ...
        compare_diff(m_current, mk1, 351, 881, "m_current", "mk");
